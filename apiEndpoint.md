...
- **Note**: Users must have an `active` status to log in. Other statuses (`pending`, `suspended`, `declined`) will return a `403 Forbidden` response.
...
- **Form-Data Body** (Required for images):
  - `mobile_number`: `09123456789`
  - `first_name`: `Juan`
  - `last_name`: `Dela Cruz`
  - `gender`: `male` (options: `male`, `female`, `other`, `prefer_not_to_say`)
  - `date_of_birth`: `1990-01-01`
  - `pin`: `123456` (6-digit string)
  - `role`: `client` or `therapist`
  - `id_card_photo`: `[File]` (Required, image, max 2MB)
  - `id_selfie_photo`: `[File]` (Required, image, max 2MB)
  - `profile_photo`: `[File]` (Optional, image, max 2MB)

#### 🚀 Postman / Curl Sample:
To test in Postman, use **form-data** instead of raw JSON.
```bash
curl --location 'http://localhost:8000/api/auth/register-profile' \
--form 'mobile_number="09123456789"' \
...
--form 'pin="123456"' \
--form 'role="client"' \
--form 'id_card_photo=@"/path/to/id_card.jpg"' \
--form 'id_selfie_photo=@"/path/to/selfie.jpg"'
```
